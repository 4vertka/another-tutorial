...
void VulkanEngine::draw() {
  ...
  vkutil::transition_image(cmd, _depthImage.image, VK_IMAGE_LAYOUT_GENERAL,
...
void VulkanEngine::create_swapchain(uint32_t widht, uint32_t height) {
  ...
  vkb::Swapchain vkbSwapchain =
      swapchainBuilder
          ...
          .set_desired_extent(widht, height)
...
void VulkanEngine::draw_background(VkCommandBuffer cmd) {
  ...
  //     vkinit::image_subresources_range(VK_IMAGE_ASPECT_COLOR_BIT);
...
void VulkanEngine::init_background_pipelines() {
  ...
  if (!vkutil::load_shader_module("../build/shaders/sky.comp.spv", _device,
                                  &skyShader)) {
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices,
  ...
  if (vertices.empty() || indices.empty()) {
    std::cerr << "Error: Attempted to upload mesh with empty data" << std::endl;
    return {}; // Return empty buffer or throw an exception
  }
  ...
  try {
    ...
    std::cerr << "Exception in uploadMesh: " << e.what() << std::endl;
...
void VulkanEngine::init_mesh_pipeline() {
  ...
  if (!vkutil::load_shader_module("../build/shaders/color_triangle.frag.spv",
                                  _device, &triangleFragShader)) {
    std::cout << "error building color_triangle.frag shaders\n";
  } else {
    std::cout << "  color_triangle.frag builder successfully\n";
  }

  VkShaderModule triangleVertexShader;
  if (!vkutil::load_shader_module(
          "../build/shaders/colored_triangle_mesh.vert.spv", _device,
...
void VulkanEngine::init_default_data() {
  ...
  //   auto testmesh = loadGltfMeshes(this, "../assets/basicmesh.glb");
}
